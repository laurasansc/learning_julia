...
function get_candidates(distance_matrix::Array, threshold::Float64, free_points::Array, calculated_points::Array)
	...
	return clusters  # RENAME TO CANDIDATE POINTS
end
...
function qt_clustering(distance_matrix::Array, threshold::Float64, candidate_clusters::Array, calculated_points::Array)
...
function update_points(top_cluster, new_clusters, selected_points, free_points, calculated_points,)

	selected_points = union(selected_points, top_cluster)
	free_points = setdiff(free_points,selected_points)
	calculated_points = setdiff(new_clusters,selected_points)

	return selected_points, calculated_points,  free_points

end

function re_make_clusters(candidate_clusters, top_cluster)
	new_clusters = []

	for i in candidate_clusters
		x = candidate_clusters[i]
		if length(intersect(top_cluster, x)) != 1
...
function QT(filename, qt)
	...
	while length(free_points) > 0
		# problem doesnt get other clusters just one then all empty
		candidate_clusters = qt_clustering(distance_matrix, threshold, clusters, calculated_points )
		println(candidate_clusters)
		# find the biggest cluster and return it 
		top_cluster = sort(get_top_cluster(candidate_clusters))

		new_clusters = re_make_clusters(candidate_clusters, top_cluster)

		selected_points, calculated_points, free_points = update_points(top_cluster, new_clusters, selected_points, free_points, calculated_points)
